...
def get_best_models_config(data, best_num=5):
    ...
    metrics = ['roc_auc_score','f1_weighted','matthews_corrcoef', 'balanced_accuracy', 'precision_weighted', 'recall_weighted', 'specificity', 'NPV']
...
def train_predict_model(save_path, y_test, y_pred, y_proba,  fold, model_name, params, total_time):
    ...
    metrics = metrics[['model', 'config', 'fold', 'roc_auc_score', 'f1_weighted','matthews_corrcoef', 'balanced_accuracy','precision_weighted','recall_weighted', 'specificity', 'NPV', 'time(minutes)']]
    return metrics